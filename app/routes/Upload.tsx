...
const Upload = () => {
    ...
    const {auth, isLoading, fs, ai, kv} = usePuterStore() // fs = fileStorage
    ...
    const handleAnalyze = async ({companyName,jobTitle,jobDescription,file}:analysisProps) => {
        ...
        console.log("The imageFile is: ",imageFile)
        if(!imageFile || !imageFile.file) return setStatusText("Failed to convert pdf to image")
        
        const safeImageFile = imageFile.file as File;

        setStatusText("Uploading the image...")
        const uploadedImage = await fs.upload([safeImageFile])
